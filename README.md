# 👋 Hi, I'm Tarun Telang

Distinguished Engineer | Architect | Engineering Leader  
Author | Educator | Mentor  
Passionate about Artificial Intelligence, Microservices, System Design, and Cloud-Native Architectures 

[Read my books on O'Reilly](https://www.oreilly.com/search/?q=author%3A%22Tarun%20Telang%22&rows=100)

---

## About Me

I’m a senior software engineering leader with 20+ years of experience working at top technology companies like Microsoft, Oracle, SAP, and BlackBerry.

I specialize in:
- Designing scalable distributed systems
- Building resilient microservices architectures
- Mentoring engineers and leading high-performing teams
- Creating practical learning content for developers worldwide

---

## What I’m Working On

- 📘 Writing **"Beginning Cloud-Native Development (2nd Edition)"**
- 🎓 Creating courses on System Design and Generative AI
- 🎥 Growing my YouTube channels:
  - DevSkills (system design, engineering concepts) - https://www.youtube.com/@dev-skills
- 🧪 Building projects around:
  - Event-driven systems
  - Saga patterns & distributed transactions
  - Cloud-native Java applications

---

## Tech Stack

**Languages & Frameworks**
- Java, Spring Boot, Jakarta EE
- MicroProfile, Axon Framework

**Architecture & Patterns**
- Microservices, Event-Driven Architecture
- CQRS, Saga Pattern, TCC
- Distributed Transactions (XA, BASE)

**Cloud & Tools**
- Docker, Kubernetes
- Kafka, ActiveMQ
- GitHub Actions, CI/CD pipelines

---

## Publications & Content

- ✍️ Blog: https://taruntelang.medium.com/
- 📘 Books on Java, YAML, Microservices & Cloud-Native Systems
- 🎥 200+ videos on DevSkills (https://www.youtube.com/@dev-skills)

---

## 🎯 Current Focus

- Helping engineers master **System Design**
- Teaching **real-world microservices patterns**
- Bridging the gap between **theory and production systems**
- Enabling leaders to adopt **Generative AI effectively**

---

## 📈 GitHub Stats

![Tarun's GitHub stats](https://github-readme-stats.vercel.app/api?username=ttelang&show_icons=true&theme=radical)

---

## 🤝 Let’s Connect

- 💼 LinkedIn: [https://linkedin.com/in/ttelang](http://linkedin.com/in/taruntelang)
- 🌐 Website: https://practicaldeveloper.com
- 📺 YouTube: https://www.youtube.com/@dev-skills

---

## ⚡ Fun Fact

I enjoy creating hands-on learning experiences that make complex concepts simple.

---

> "Great architecture is not just about technology. It’s about making systems resilient, scalable, and understandable."

